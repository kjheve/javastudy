...
public class Bank {
    // 계좌관리 - 최대치 5개
    private static Account[] accounts; // 인스턴스마다 생성X, 공유가 되어야해서 static
    ...
    // 🎈★폐지
    private void closingAccout(String accountNumber) {
        // 계좌번호로 해당 계좌 찾아오기 🍀
        ...
        if (account.getBalance() > 0) {
            System.out.println("[🚨]잔액이 남은 계좌는 폐지 불가능");
            return;
        }
    ...
    private void listAccount() {
        ...
        System.out.println("- 잔여 계좌수 : " + (accounts.length - usingAccount)); // 상수 받던거에서 accounts 배열의 길이를 받음
    }
